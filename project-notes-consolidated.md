...
## Header layout & mobile responsiveness (PRs #12–#14, #16–#17, #19, #21–#24)
...
11. **8 new venues added to `shows.json` 2026-09-12, still no shows scraped for any of them** — metadata only (name/site/type/color/address). A same-day on-demand fetch attempt came back empty for all 8 (see "New venues added 2026-09-12" below for why: thin venue pages plus a `livemusiccharleston.com` source that's a documented dead end for this routine's fetch tooling). Needs either those venues to start publishing their own listings, or a fix for the `livemusiccharleston.com` fetch (browser-like headers, a different client, etc. — same open question as item 1 below for AXS/Bandsintown).
...
**2026-09-12 on-demand run — first actual fetch attempt, zero shows recovered for any of the 8.** The allowlist update landed (all 8 venue domains plus `livemusiccharleston.com` returned HTTP 200 via direct curl — no `EGRESS_BLOCKED`), but every source came back with no usable event data:

- `US`, `TC`, `DIN`, `RSG`, `CHA`, `SNOB` — each venue's own page is a thin restaurant-platform stub (Uptown Social explicitly says "powered by BentoBox"; Dinghy is Wix) whose entire "Live Music"/"Entertainment" section is one sentence pointing at Live Music Charleston, with zero inline listings. Confirmed by direct inspection, not an assumption — there's nothing here to scrape until/unless a venue adds its own listings.
- `TMPC`/`TMJI` — the venue's own site does have a real `/events` page (Squarespace), but it hasn't been updated since ~September 2025: every dated, named-act entry on it is 2025 and already in the past. The only entry reaching into the future is a generic recurring "Weekend Live Music" placeholder (no act named, spanning mid-2026 to mid-2028) — not usable as a dated show record. The page itself also points to Live Music Charleston for the actual current bookings at both locations.
- `livemusiccharleston.com` — confirmed to be a client-side-rendered SPA, not usable by this routine's fetch mechanism. Direct curl returns HTTP 200 but only the `<title>` tag and empty markup (checked on the Tattooed Moose and Uptown Social venue pages). A headless-Chromium render was also attempted (Playwright + the sandbox's pre-installed Chromium) to work around the JS-rendering gap; that failed differently and further upstream — the navigation itself was hit with `ERR_CONNECTION_RESET` at the proxy layer before any page content loaded, on every attempt. So this source is a documented dead end for the routine's current tooling, same category as the AXS/Bandsintown bot-detection issues already tracked below, though the failure mode is distinct (connection reset / JS shell, not a 403).

Net result: `shows.json` is unchanged this run — there is nothing real to add yet for any of these 8 venues, and nothing to correct since none of them have existing entries. Re-attempting `livemusiccharleston.com` on a future run is worth doing (both failure modes could be client- or network-context-specific), but per the standing "don't keep re-diagnosing a documented limitation every run" convention, don't re-investigate it from scratch each time — just note if a future attempt behaves differently. If any of these venues starts publishing its own dated, named-act listings directly (rather than outsourcing to Live Music Charleston), that becomes the primary source going forward, per the normal source-priority rule.
